...
local Linux = {
    Theme = {
        Background = Color3.fromRGB(20, 20, 20), -- Escurecido (era 24, 24, 24)
        Element = Color3.fromRGB(25, 25, 25), -- Escurecido (era 28, 28, 28)
        Accent = Color3.fromRGB(255, 182, 193), -- Rosa claro (aproximado do 🩷, mas mais suave)
        Text = Color3.fromRGB(180, 180, 180),
        Toggle = Color3.fromRGB(40, 40, 40),
        TabInactive = Color3.fromRGB(25, 25, 25), -- Escurecido (era 28, 28, 28)
        DropdownOption = Color3.fromRGB(30, 30, 30),
        SelectedOption = Color3.fromRGB(255, 215, 0)
    }
}
...
function Linux.Create(config)
    ...
    local ToggleIconGui = Linux.Instance("ScreenGui", {
        Name = "ToggleIconGui",
        Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui"),
        ResetOnSpawn = false,
        Enabled = config.Icon and config.Icon.Enabled ~= nil and config.Icon.Enabled or true
    })

    ProtectGui(ToggleIconGui)

    local ToggleIconButton = Linux.Instance("ImageButton", {
        Parent = ToggleIconGui,
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 50, 0, 50),
        Position = isMobile and UDim2.new(0, 10, 0, 10) or UDim2.new(0, 20, 0, 20),
        Image = config.Icon and config.Icon.Image or "rbxassetid://10747384394",
        ZIndex = 10
    })

    Linux.Instance("UICorner", {
        Parent = ToggleIconButton,
    ...
    local SubtitleLabel = Linux.Instance("TextLabel", {
        ...
        TextColor3 = Color3.fromRGB(100, 100, 100), -- Cor igual às abas não selecionadas
        TextSize = 14, -- Menor que o título (era 16)
        TextXAlignment = Enum.TextXAlignment.Left,
        AutomaticSize = Enum.AutomaticSize.X,
        ZIndex = 2
    })
    ...
    ToggleIconButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            draggingIcon = true
            dragStartPos = input.Position
            iconStartPos = ToggleIconButton.Position
        end
    end)

    ToggleIconButton.InputChanged:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and draggingIcon then
            ...
            local newPosX = iconStartPos.X.Offset + delta.X
            local newPosY = iconStartPos.Y.Offset + delta.Y
            ToggleIconButton.Position = UDim2.new(0, newPosX, 0, newPosY)
        end
    end)

    ToggleIconButton.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            draggingIcon = false
            dragStartPos = nil
            iconStartPos = nil
    ...
    function LinuxLib.Tab(config)
        ...
        local TabSeperator = Linux.Instance("Frame", {
            Name = "TabSeperator",
            Parent = TabBtn,
            BackgroundColor3 = Linux.Theme.Accent, -- Rosa claro
        ...
        local function SelectTab()
            for _, tab in pairs(Tabs) do
                ...
                tab.Button.TabSeperator.Size = UDim2.new(0, 0, 0.8, 0)
            end
            ...
            TabSeperator.Size = UDim2.new(0, 4, 0.8, 0)
            CurrentTab = tabIndex
        end
        ...
        function TabElements.Button(config)
            ...
            local originalColor = Linux.Theme.Element
        ...
        function TabElements.Toggle(config)
            ...
            local function UpdateToggle()
                ...
                if State then
                    ...
                    TweenService:Create(Knob, tween, {Position = UDim2.new(1, -18, 0, 2), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0}):Play()
                else
                    TweenService:Create(ToggleFill, tween, {BackgroundColor3 = Linux.Theme.Toggle, Size = UDim2.new(0, 0, 1, 0)}):Play()
                    TweenService:Create(Knob, tween, {Position = UDim2.new(0, 2, 0, 2), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0}):Play()
                end
            end
